<template>
  <el-form label-position="top" size="small" :model="formData">
    <!-- 数据库类型 -->
    <template v-if="isDbType">
      <DataSourceSelector :db-type="nodeSubType" @select="onDataSourceSelect" />
      <el-divider v-if="formData.datasourceId" content-position="left">
        <span style="font-size: 12px; color: #999">已选择数据源，可手动修改</span>
      </el-divider>
      <el-form-item label="JDBC URL">
        <el-input v-model="formData.url" placeholder="jdbc:mysql://localhost:3306/db" @change="emitUpdate" />
      </el-form-item>
      <el-form-item label="用户名">
        <el-input v-model="formData.username" @change="emitUpdate" />
      </el-form-item>
      <el-form-item label="密码">
        <el-input v-model="formData.password" type="password" show-password @change="emitUpdate" />
      </el-form-item>
      <el-form-item label="表名">
        <el-input v-model="formData.table" placeholder="table_name" @change="emitUpdate" />
      </el-form-item>
      <el-form-item label="自定义 SQL（可选）">
        <el-input v-model="formData.query" type="textarea" :rows="3" placeholder="SELECT * FROM table WHERE ..." @change="emitUpdate" />
      </el-form-item>
    </template>

    <!-- CSV -->
    <template v-else-if="nodeSubType === 'csv'">
      <el-form-item label="文件路径">
        <el-input v-model="formData.filePath" placeholder="/data/input.csv" @change="emitUpdate" />
      </el-form-item>
      <el-form-item label="分隔符">
        <el-input v-model="formData.delimiter" placeholder="," @change="emitUpdate" />
      </el-form-item>
      <el-form-item label="编码">
        <el-select v-model="formData.encoding" @change="emitUpdate">
          <el-option label="UTF-8" value="UTF-8" />
          <el-option label="GBK" value="GBK" />
          <el-option label="ISO-8859-1" value="ISO-8859-1" />
        </el-select>
      </el-form-item>
      <el-form-item label="首行为表头">
        <el-switch v-model="formData.hasHeader" @change="emitUpdate" />
      </el-form-item>
    </template>

    <!-- Excel -->
    <template v-else-if="nodeSubType === 'excel'">
      <el-form-item label="文件路径">
        <el-input v-model="formData.filePath" placeholder="/data/input.xlsx" @change="emitUpdate" />
      </el-form-item>
      <el-form-item label="Sheet 索引">
        <el-input-number v-model="formData.sheetIndex" :min="0" @change="emitUpdate" />
      </el-form-item>
      <el-form-item label="首行为表头">
        <el-switch v-model="formData.hasHeader" @change="emitUpdate" />
      </el-form-item>
    </template>

    <!-- JSON -->
    <template v-else-if="nodeSubType === 'json'">
      <el-form-item label="文件路径">
        <el-input v-model="formData.filePath" placeholder="/data/input.json" @change="emitUpdate" />
      </el-form-item>
    </template>

    <!-- FTP/SFTP -->
    <template v-else-if="nodeSubType === 'ftp' || nodeSubType === 'sftp'">
      <el-form-item label="主机">
        <el-input v-model="formData.host" @change="emitUpdate" />
      </el-form-item>
      <el-form-item label="端口">
        <el-input-number v-model="formData.port" :min="1" :max="65535" @change="emitUpdate" />
      </el-form-item>
      <el-form-item label="用户名">
        <el-input v-model="formData.username" @change="emitUpdate" />
      </el-form-item>
      <el-form-item label="密码">
        <el-input v-model="formData.password" type="password" show-password @change="emitUpdate" />
      </el-form-item>
      <el-form-item label="文件路径">
        <el-input v-model="formData.filePath" @change="emitUpdate" />
      </el-form-item>
      <el-form-item label="文件类型">
        <el-select v-model="formData.fileType" @change="emitUpdate">
          <el-option label="CSV" value="csv" />
          <el-option label="JSON" value="json" />
        </el-select>
      </el-form-item>
    </template>
  </el-form>
</template>

<script setup lang="ts">
import { reactive, computed, watch } from 'vue'
import DataSourceSelector from './DataSourceSelector.vue'
import type { DataSourceConfig } from '@/api/datasource'

interface Props {
  nodeSubType: string
  config: Record<string, any>
}

const props = defineProps<Props>()
const emit = defineEmits<{
  (e: 'update', config: Record<string, any>): void
}>()

const isDbType = computed(() => ['mysql', 'postgresql', 'oracle', 'hive'].includes(props.nodeSubType))

const formData = reactive<Record<string, any>>({
  url: '',
  username: '',
  password: '',
  table: '',
  query: '',
  datasourceId: undefined,
  filePath: '',
  delimiter: ',',
  encoding: 'UTF-8',
  hasHeader: true,
  sheetIndex: 0,
  host: '',
  port: props.nodeSubType === 'sftp' ? 22 : 21,
  fileType: 'csv',
  ...props.config,
})

// 选择已有数据源时自动填充
function onDataSourceSelect(ds: DataSourceConfig) {
  formData.datasourceId = ds.id
  formData.url = ds.url || ''
  formData.username = ds.username || ''
  // 密码不回填（安全考虑）
  emitUpdate()
}

function emitUpdate() {
  emit('update', { ...formData })
}

watch(() => props.config, (newConfig) => {
  Object.assign(formData, newConfig)
}, { deep: true })
</script>
