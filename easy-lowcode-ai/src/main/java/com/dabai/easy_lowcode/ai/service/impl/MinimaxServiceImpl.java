...
@Slf4j
@Service
@ConditionalOnProperty(name = "ai.provider.default", havingValue = "minimax")
public class MinimaxServiceImpl implements AiService {
    ...
    @Value("${ai.minimax.model:abab6-chat}")
    private String defaultModel;
    
    private static final String API_URL_SUFFIX = "/text/chatcompletion";
    ...
    public ChatResponse chat(ChatRequest request) {
        ...
        try {
            ...
            if (!response.isOk()) {
                throw new RuntimeException("Minimax API 调用失败: " + response.body());
            }
