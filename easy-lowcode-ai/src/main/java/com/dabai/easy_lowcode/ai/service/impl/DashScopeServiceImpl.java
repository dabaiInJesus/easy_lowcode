...
public class DashScopeServiceImpl implements AiService {
    ...
    public ChatResponse chat(ChatRequest request) {
        ...
        AiCallLogger.logRequest(AiProvider.DASHSCOPE, "dashscope", request.getMessage(), request.getSystemPrompt());
        ...
        try {
            ...
            AiCallLogger.logResponse(AiProvider.DASHSCOPE, "dashscope", elapsed,
                    chatResponse.getContent() != null ? chatResponse.getContent().length() : 0, true);
            return chatResponse;

        } catch (Exception e) {
            long elapsed = System.currentTimeMillis() - start;
            AiCallLogger.logResponse(AiProvider.DASHSCOPE, "dashscope", elapsed, 0, false);
    ...
    public Flux<String> streamChat(ChatRequest request) {
        ensureAvailable();
        long start = System.currentTimeMillis();
        AiCallLogger.logStreamStart(AiProvider.DASHSCOPE, "dashscope", request.getMessage());
        ...
        try {
            ...
            return dashScopeChatModel.stream(prompt)
                    ...
                    .doOnComplete(() -> AiCallLogger.logStreamEnd(AiProvider.DASHSCOPE,
                            System.currentTimeMillis() - start, (int) chunkCount[0], true))
                    .doOnError(e -> AiCallLogger.logStreamEnd(AiProvider.DASHSCOPE,
                            System.currentTimeMillis() - start, (int) chunkCount[0], false));

        } catch (Exception e) {
            AiCallLogger.logStreamEnd(AiProvider.DASHSCOPE, System.currentTimeMillis() - start, 0, false);
